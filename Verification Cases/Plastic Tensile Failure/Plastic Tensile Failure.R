...
CorePath <- "C:/Users/mhort/OneDrive/Documents/Lammps/lammps-stable/lammps-2Aug2023/Testing/Verification Cases/Plastic Tensile Failure/"
library(ggplot2)
library(dplyr)
library(data.table)
...
dffiltered<- df %>%
filter(S<max(StressStrainData$S))

fig<- ggplot() +
  geom_line(data=dffiltered, aes(x=e, y=S)) +
