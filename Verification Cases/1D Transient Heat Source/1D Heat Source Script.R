...
CorePath <- "C:/Users/mhort/OneDrive/Documents/Lammps/lammps-stable/lammps-2Aug2023/Testing/Verification Cases/1D Transient Heat Source/"
library(ggplot2)
library(dplyr)
library(data.table)
...
Qget<-function(x,t,rho,kappa,mmax,L,cp,qdot){
  Qfinal=0
  for(m in 1:mmax){
    Qfinal= Qfinal +((4*L*((-1)^(m)))/((pi^2)*(((2*m)-1)^2))) * sin(((2*m)-1)*pi*x/L) * exp((-1*(((2*m)-1)^2)*(pi^2)*(kappa/(rho*cp))*t)/(L^2))
  }
  Qfinal= Qfinal + x
  Qfinal=Qfinal*qdot
  return(Qfinal)
...
for (i in t){
  ...
  for(j in x){
    ...
    Qarray[length(Qarray)+1]<- Qget(j,i,rho,kappa,mmax,L,cp,qdot)
  }
...
errorget = function(dflammps,T0,kappa,mmax,L,rho,cp,qdot){
  
  dflammps<- dflammps %>%
    mutate(TError = c_t - Tget(x,as.numeric(t),rho,kappa,mmax,L,cp,qdot)) %>%
    mutate(QError = c_Q1 - Qget(x,as.numeric(t),rho,kappa,mmax,L,cp,qdot)) %>%
...
dflammps <- errorget(dflammps,T0,kappa,mmax,L,rho,cp,qdot)
