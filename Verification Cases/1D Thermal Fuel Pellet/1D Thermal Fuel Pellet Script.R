...
CorePath <- "C:/Users/mhort/OneDrive/Documents/Lammps/lammps-stable/lammps-2Aug2023/Testing/Verification Cases/1D Thermal Fuel Pellet/"
library(ggplot2)
library(dplyr)
library(data.table)
...
t = c(30000*0.01) #just end time of file
kf=1
kc=10
qdot = 1
Tinf = 300
...
Tget<-function(x,Tinf,h,Lf,Lc,kf,kc,qdot){
  ...
  for(i in x){
  if(i<Lf){
    Tfinal=c(Tfinal,qdot*(Lf/h + (Lc*Lf-Lf^2)/kc + (Lf^2 - i^2)/(2*kf)) + Tinf)
  }else{
    Tfinal=c(Tfinal,qdot*Lf*(1/h + (Lc-i)/kc) + Tinf)
  }
    }
...
Qget<-function(qdot,Lf,x){
  ...
  for(i in x){
    if(i<Lf){
      Qfinal=c(Qfinal,qdot*i)
    }else{
      Qfinal=c(Qfinal,qdot*Lf)
    }
  }
  return(Qfinal)
}
...
errorget = function(dflammps,x,Tinf,h,Lf,Lc,kf,kc,qdot){
  ...
  dflammps<- dflammps %>%
    mutate(TError = c_t - Tget(x,Tinf,h,Lf,Lc,kf,kc,qdot)) %>%
    mutate(QError = c_Q1 - Qget(qdot,Lf,x)) %>%
...
totalmassarray[icount]=n*(dp^2)

width = floor(6/dp)
length = floor(Lf/dp)+1
print(n-length*width)
correctedheatarray[icount]=6/(width*dp)
