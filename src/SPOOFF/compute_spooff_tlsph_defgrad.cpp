...
#include "compute_spooff_tlsph_defgrad.h"
...
ComputeSPOOFFTLSPHDefgrad::ComputeSPOOFFTLSPHDefgrad(LAMMPS *lmp, int narg, char **arg) :
                Compute(lmp, narg, arg) {
        if (narg != 3)
                error->all(FLERR, "Illegal compute spooff/tlsph_defgrad command");
...
ComputeSPOOFFTLSPHDefgrad::~ComputeSPOOFFTLSPHDefgrad() {
        memory->sfree(defgradVector);
}

/* ---------------------------------------------------------------------- */

void ComputeSPOOFFTLSPHDefgrad::init() {

        int count = 0;
        for (int i = 0; i < modify->ncompute; i++)
                if (strcmp(modify->compute[i]->style, "spooff/tlsph_defgrad") == 0)
                        count++;
        if (count > 1 && comm->me == 0)
                error->warning(FLERR, "More than one compute spooff/tlsph_defgrad");
}

/* ---------------------------------------------------------------------- */

void ComputeSPOOFFTLSPHDefgrad::compute_peratom() {
...
double ComputeSPOOFFTLSPHDefgrad::memory_usage() {
