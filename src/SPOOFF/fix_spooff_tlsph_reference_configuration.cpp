...
FixSPOOFF_TLSPH_ReferenceConfiguration::FixSPOOFF_TLSPH_ReferenceConfiguration(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg) {
...
FixSPOOFF_TLSPH_ReferenceConfiguration::~FixSPOOFF_TLSPH_ReferenceConfiguration() {
...
int FixSPOOFF_TLSPH_ReferenceConfiguration::setmask() {
  int mask = 0;
  mask |= PRE_EXCHANGE;
  return mask;
}
...
void FixSPOOFF_TLSPH_ReferenceConfiguration::init() {
  if (atom->tag_enable == 0) error->all(FLERR, "Pair style tlsph requires atoms have IDs");
}

/* ---------------------------------------------------------------------- */

void FixSPOOFF_TLSPH_ReferenceConfiguration::pre_exchange() {
  ...
  int *updateFlag_ptr = (int *) force->pair->extract("spooff/tlsph/updateFlag_ptr", itmp);
  if (updateFlag_ptr == nullptr) {
    error->one(FLERR,
               "fix FixSPOOFF_TLSPH_ReferenceConfiguration failed to access updateFlag pointer. Check if a pair style exist which calculates this quantity.");
  }

  int *nn = (int *) force->pair->extract("spooff/tlsph/numNeighsRefConfig_ptr", itmp);
  if (nn == nullptr) {
    error->all(FLERR, "FixSPOOFFIntegrateTlsph::updateReferenceConfiguration() failed to access numNeighsRefConfig_ptr array");
  }
...
void FixSPOOFF_TLSPH_ReferenceConfiguration::setup(int /*vflag*/) {
...
double FixSPOOFF_TLSPH_ReferenceConfiguration::memory_usage() {
  int nmax = atom->nmax;
  int bytes = nmax * sizeof(int);
  bytes += (double)nmax * maxpartner * sizeof(tagint); // partner array
  bytes += (double)nmax * maxpartner * sizeof(tagint); // partnerfirst array
  ...
  bytes += (double)nmax * maxpartner * sizeof(float); // impact velocity array
...
void FixSPOOFF_TLSPH_ReferenceConfiguration::grow_arrays(int nmax) {
  ...
  memory->grow(partnerfirst, nmax, maxpartner, "tlsph_refconfig_neigh:partnerfirst");
  ...
  memory->grow(impact_velocity, nmax, maxpartner, "tlsph_refconfig_neigh:impact_velocity");
...
void FixSPOOFF_TLSPH_ReferenceConfiguration::copy_arrays(int i, int j, int /*delflag*/) {
  npartner[j] = npartner[i];
  for (int m = 0; m < npartner[j]; m++) {
    partner[j][m] = partner[i][m];
    partnerfirst[j][m] = partnerfirst[i][m];
    wfd_list[j][m] = wfd_list[i][m];
    wf_list[j][m] = wf_list[i][m];
    degradation_ij[j][m] = degradation_ij[i][m];
    impact_velocity[j][m] = impact_velocity[i][m];
...
int FixSPOOFF_TLSPH_ReferenceConfiguration::pack_exchange(int i, double *buf) {
...
// Comm::BUFEXTRA may need to be increased
  ...
  for (int n = 0; n < npartner[i]; n++) {
    buf[m++] = partner[i][n];
    buf[m++] = partnerfirst[i][n];
    buf[m++] = wfd_list[i][n];
    buf[m++] = wf_list[i][n];
    buf[m++] = degradation_ij[i][n];
    buf[m++] = impact_velocity[i][n];
...
int FixSPOOFF_TLSPH_ReferenceConfiguration::unpack_exchange(int nlocal, double *buf) {
  if (nlocal == nmax) {
  ...
  for (int n = 0; n < npartner[nlocal]; n++) {
    ...
    partnerfirst[nlocal][n] = static_cast<tagint>(buf[m++]);
    ...
    impact_velocity[nlocal][n] = static_cast<float>(buf[m++]);
...
int FixSPOOFF_TLSPH_ReferenceConfiguration::pack_restart(int i, double *buf) {
  int m = 0;
...
void FixSPOOFF_TLSPH_ReferenceConfiguration::unpack_restart(int /*nlocal*/, int /*nth*/) {
...
int FixSPOOFF_TLSPH_ReferenceConfiguration::maxsize_restart() {
...
int FixSPOOFF_TLSPH_ReferenceConfiguration::size_restart(int nlocal) {
  return 4 * npartner[nlocal] + 2;
}

/* ---------------------------------------------------------------------- */

int FixSPOOFF_TLSPH_ReferenceConfiguration::pack_forward_comm(int n, int *list, double *buf, int /*pbc_flag*/, int * /*pbc*/) {
  int i, j, m;
  double *radius = atom->radius;
  double *vfrac = atom->vfrac;
  double **x0 = atom->x0;
...
void FixSPOOFF_TLSPH_ReferenceConfiguration::unpack_forward_comm(int n, int first, double *buf) {
  int i, m, last;
  double *radius = atom->radius;
  double *vfrac = atom->vfrac;
  double **x0 = atom->x0;
...
bool FixSPOOFF_TLSPH_ReferenceConfiguration::crack_exclude(int i, int j) {
