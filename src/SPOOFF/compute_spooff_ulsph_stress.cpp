...
#include "compute_spooff_ulsph_stress.h"
#include <cmath>
#include <cstring>
#include <Eigen/Eigen>
#include "atom.h"
...
ComputeSPOOFFULSPHStress::ComputeSPOOFFULSPHStress(LAMMPS *lmp, int narg, char **arg) :
                Compute(lmp, narg, arg) {
        if (narg != 3)
                error->all(FLERR, "Illegal compute spooff/ulsph_stress command");
...
ComputeSPOOFFULSPHStress::~ComputeSPOOFFULSPHStress() {
        memory->sfree(stress_array);
}

/* ---------------------------------------------------------------------- */

void ComputeSPOOFFULSPHStress::init() {

        int count = 0;
        for (int i = 0; i < modify->ncompute; i++)
                if (strcmp(modify->compute[i]->style, "spooff/ulsph_stress") == 0)
                        count++;
        if (count > 1 && comm->me == 0)
                error->warning(FLERR, "More than one compute spooff/ulsph_stress");
}

/* ---------------------------------------------------------------------- */

void ComputeSPOOFFULSPHStress::compute_peratom() {
        ...
        int itmp = 0;
        auto T = (Matrix3d *) force->pair->extract("spooff/ulsph/stressTensor_ptr", itmp);
        if (T == nullptr) {
                error->all(FLERR, "compute spooff/ulsph_stress could not access stress tensors. Are the matching pair styles present?");
        }
...
double ComputeSPOOFFULSPHStress::memory_usage() {
