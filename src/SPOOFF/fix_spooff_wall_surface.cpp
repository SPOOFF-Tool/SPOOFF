...
#include "fix_spooff_wall_surface.h"
...
FixSPOOFFWallSurface::FixSPOOFFWallSurface(LAMMPS *lmp, int narg, char **arg) :
                Fix(lmp, narg, arg) {
        ...
        if (narg != 6)
                error->all(FLERR, "Illegal number of arguments for fix spooff/wall_surface");
        ...
        if (comm->me == 0) {
                printf("\n>>========>>========>>========>>========>>========>>========>>========>>========\n");
                printf("fix spooff/wall_surface reads trianglulated surface from file: %s\n", filename);
                printf("fix spooff/wall_surface has particle type %d \n", wall_particle_type);
                printf("fix spooff/wall_surface has molecule id %d \n", wall_molecule_id);
                printf(">>========>>========>>========>>========>>========>>========>>========>>========\n");
        }
}
...
FixSPOOFFWallSurface::~FixSPOOFFWallSurface() {
        free(filename);
        filename = nullptr;
...
int FixSPOOFFWallSurface::setmask() {
        int mask = 0;
        return mask;
}
...
void FixSPOOFFWallSurface::init() {
        if (!first)
                return;
}
...
void FixSPOOFFWallSurface::min_setup(int vflag) {
        setup(vflag);
}
...
void FixSPOOFFWallSurface::setup(int /*vflag*/) {
...
void FixSPOOFFWallSurface::read_triangles(int pass) {
  ...
  // print status
  if (comm->me == 0) {
    utils::logmesg(lmp, "... fix spooff/wall_surface finished reading triangulated surface\n");
    utils::logmesg(lmp, "fix spooff/wall_surface created {} atoms\n", atom->natoms - natoms_previous);
    utils::logmesg(lmp, ">>========>>========>>========>>========>>========>>========\n");
  }
