...
FixSPOOFFSetVel::FixSPOOFFSetVel(LAMMPS *lmp, int narg, char **arg) :
...
FixSPOOFFSetVel::~FixSPOOFFSetVel()
{
  delete[] xstr;
  delete[] ystr;
  delete[] zstr;
...
void FixSPOOFFSetVel::setup(int vflag)
{
  if (utils::strmatch(update->integrate_style, "^verlet"))
    post_force(vflag);
  else
    error->all(FLERR, "Fix spooff/setvel does not support RESPA");
}

/* ---------------------------------------------------------------------- */

void FixSPOOFFSetVel::min_setup(int vflag)
{
  post_force(vflag);
}
...
void FixSPOOFFSetVel::post_force(int /*vflag*/)
{
  double **x = atom->x;
  double **f = atom->f;
  double **v = atom->v;
...
double FixSPOOFFSetVel::compute_vector(int n)
{
...
double FixSPOOFFSetVel::memory_usage()
{
  double bytes = 0.0;
