...
#include "compute_spooff_tlsph_shape.h"
...
ComputeSpooffTlsphShape::ComputeSpooffTlsphShape(LAMMPS *lmp, int narg, char **arg) :
...
ComputeSpooffTlsphShape::~ComputeSpooffTlsphShape() {
        memory->sfree(strainVector);
}

/* ---------------------------------------------------------------------- */

void ComputeSpooffTlsphShape::init() {
...
void ComputeSpooffTlsphShape::compute_peratom() {
        ...
        int itmp = 0;
        auto R = (Matrix3d *) force->pair->extract("spooff/tlsph/rotation_ptr", itmp);
        if (R == nullptr) {
                error->all(FLERR, "compute spooff/tlsph_shape failed to access rotation array");
        }

        auto F = (Matrix3d *) force->pair->extract("spooff/tlsph/Fincr_ptr", itmp);
        if (F == nullptr) {
                error->all(FLERR, "compute spooff/tlsph_shape failed to access deformation gradient array");
        }
...
double ComputeSpooffTlsphShape::memory_usage() {
