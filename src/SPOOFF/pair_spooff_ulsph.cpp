...
void PairULSPH::settings(int narg, char **arg) {
        ...
        if (comm->me == 0) {
                printf("\n>>========>>========>>========>>========>>========>>========>>========>>========\n");
                printf("... SPOOFF / ULSPH PROPERTIES\n\n");
        }
...
void PairULSPH::coeff(int narg, char **arg) {
        ...
        if (utils::inumeric(FLERR, arg[0], false, lmp) == utils::inumeric(FLERR, arg[1], false, lmp)) {
                ...
                if (comm->me == 0) {
                        printf("\n>>========>>========>>========>>========>>========>>========>>========>>========\n");
                        printf("...SPOOFF / ULSPH PROPERTIES OF PARTICLE TYPE %d\n\n", itype);
                }
                ...
                if (comm->me == 0) {
                        printf("material unspecific properties for SPOOFF/ULSPH definition of particle type %d:\n", itype);
...
//printf("in extract\n");
        if (strcmp(str, "spooff/ulsph/smoothVel_ptr") == 0) {
                return (void *) smoothVel;
        } else if (strcmp(str, "spooff/ulsph/stressTensor_ptr") == 0) {
                return (void *) stressTensor;
        } else if (strcmp(str, "spooff/ulsph/velocityGradient_ptr") == 0) {
                return (void *) L;
        } else if (strcmp(str, "spooff/ulsph/numNeighs_ptr") == 0) {
                return (void *) numNeighs;
        } else if (strcmp(str, "spooff/ulsph/dtCFL_ptr") == 0) {
//printf("dtcfl = %f\n", dtCFL);
                return (void *) &dtCFL;
        } else if (strcmp(str, "spooff/ulsph/updateFlag_ptr") == 0) {
                return (void *) &updateFlag;
        } else if (strcmp(str, "spooff/ulsph/effective_modulus_ptr") == 0) {
                return (void *) effm;
        } else if (strcmp(str, "spooff/ulsph/shape_matrix_ptr") == 0) {
                return (void *) K;
        }
