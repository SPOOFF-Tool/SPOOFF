...
#include "fix_spooff_adjust_dt.h"
...
FixSPOOFFTlsphDtReset::FixSPOOFFTlsphDtReset(LAMMPS *lmp, int narg, char **arg) :
                Fix(lmp, narg, arg) {
        if (narg != 4) error->all(FLERR, "Illegal fix spooff/adjust_dt command");
...
int FixSPOOFFTlsphDtReset::setmask() {
        int mask = 0;
...
void FixSPOOFFTlsphDtReset::init() {
        dt = update->dt;
}

/* ---------------------------------------------------------------------- */

void FixSPOOFFTlsphDtReset::setup(int /*vflag*/) {
        end_of_step();
}

/* ---------------------------------------------------------------------- */

void FixSPOOFFTlsphDtReset::initial_integrate(int /*vflag*/) {
...
void FixSPOOFFTlsphDtReset::end_of_step() {
        double dtmin = BIG;
        int itmp = 0;
        ...
        auto dtCFL_TLSPH = (double *) force->pair->extract("spooff/tlsph/dtCFL_ptr", itmp);
        auto dtCFL_ULSPH = (double *) force->pair->extract("spooff/ulsph/dtCFL_ptr", itmp);
        auto dt_TRI = (double *) force->pair->extract("spooff/tri_surface/stable_time_increment_ptr", itmp);
        auto dt_HERTZ = (double *) force->pair->extract("spooff/hertz/stable_time_increment_ptr", itmp);
        auto dt_PERI_IPMB = (double *) force->pair->extract("spooff/peri_ipmb/stable_time_increment_ptr", itmp);

        if ((dtCFL_TLSPH == nullptr) && (dtCFL_ULSPH == nullptr) && (dt_TRI == nullptr) && (dt_HERTZ == nullptr)
                        && (dt_PERI_IPMB == nullptr)) {
                error->all(FLERR, "fix spooff/adjust_dt failed to access a valid dtCFL");
        }
...
double FixSPOOFFTlsphDtReset::compute_scalar() {
        return t_elapsed;
}
...
void FixSPOOFFTlsphDtReset::write_restart(FILE *fp) {
        int n = 0;
        double list[1];
        list[n++] = t_elapsed;
...
void FixSPOOFFTlsphDtReset::restart(char *buf) {
        int n = 0;
