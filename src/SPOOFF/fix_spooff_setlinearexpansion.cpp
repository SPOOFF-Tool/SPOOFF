...
FixSPOOFFSetLinearExpansion::FixSPOOFFSetLinearExpansion(LAMMPS *lmp, int narg, char **arg) :
    Fix(lmp, narg, arg), estr(nullptr), idregion(nullptr),
    region(nullptr), sforce(nullptr)
{
  if (narg < 5) error->all(FLERR, "Illegal fix setlinearexpansion command");
  ...
  if (strstr(arg[3], "v_") == arg[3]) {
    ...
    ktype = utils::numeric(FLERR, arg[4], false, lmp);  //different linearexpansion types are currently: 0 constant, 1 is UO2 correlation, 2 zirrconium correlation UNITS ARE dimensionless
  } else if (strcmp(arg[3], "NULL") == 0) {
    estyle = NONE;
  } else {
    scale = utils::numeric(FLERR, arg[3], false, lmp);
    ktype = utils::numeric(FLERR, arg[4], false, lmp);  //different linearexpansion types are currently: 0 constant, 1 is UO2 correlation, 2 zirrconium correlation
    estyle = CONSTANT;
  }
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "region") == 0) {
      if (iarg + 2 > narg) error->all(FLERR, "Illegal fix setlinearexpansion command");
      region = domain->get_region_by_id(arg[iarg + 1]);
      if (!region) error->all(FLERR, "Region {} for fix setlinearexpansion does not exist", arg[iarg + 1]);
      idregion = utils::strdup(arg[iarg + 1]);
      iarg += 2;
    } else
      error->all(FLERR, "Illegal fix setlinearexpansion command");
  }
  ...
  maxatom = atom->nmax;
  memory->create(sforce, maxatom, 3, "setlinearexpansion:sforce");
}

/* ---------------------------------------------------------------------- */

FixSPOOFFSetLinearExpansion::~FixSPOOFFSetLinearExpansion()
{
  delete[] estr;
  delete[] idregion;
...
int FixSPOOFFSetLinearExpansion::setmask()
{
  int mask = 0;
  mask |= POST_FORCE;
  return mask;
...
void FixSPOOFFSetLinearExpansion::init()
{
  // check variables

  if (estr) {
    evar = input->variable->find(estr);
    if (evar < 0) error->all(FLERR, "Variable name for fix setlinearexpansion does not exist");
    if (input->variable->equalstyle(evar))
      estyle = EQUAL;
    else if (input->variable->atomstyle(evar))
      estyle = ATOM;
    else
      error->all(FLERR, "Variable for fix setlinearexpansion is invalid style");
  }
  ...
  if (idregion) {
    ...
    if (!region) error->all(FLERR, "Region {} for fix setlinearexpansion does not exist", idregion);
  }
...
void FixSPOOFFSetLinearExpansion::setup(int vflag)
{
  if (utils::strmatch(update->integrate_style, "^verlet"))
    post_force(vflag);
  else
    error->all(FLERR, "Fix spooff/setlinearexpansion does not support RESPA");
}

/* ---------------------------------------------------------------------- */

void FixSPOOFFSetLinearExpansion::min_setup(int vflag)
{
  post_force(vflag);
}
...
void FixSPOOFFSetLinearExpansion::post_force(int /*vflag*/)
{
  double **x = atom->x;
  double **f = atom->f;
  double **v = atom->v;
...
double FixSPOOFFSetLinearExpansion::compute_vector(int n)
{
...
double FixSPOOFFSetLinearExpansion::memory_usage()
{
  double bytes = 0.0;
