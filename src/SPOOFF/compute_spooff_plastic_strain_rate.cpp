...
#include "compute_spooff_plastic_strain_rate.h"
#include <cstring>
#include "atom.h"
#include "update.h"
#include "modify.h"
...
ComputeSPOOFFPlasticStrainRate::ComputeSPOOFFPlasticStrainRate(LAMMPS *lmp, int narg, char **arg) :
  Compute(lmp, narg, arg)
{
  if (narg != 3) error->all(FLERR,"Illegal compute spooff/plastic_strain command");
  if (atom->eff_plastic_strain_rate_flag != 1) error->all(FLERR,"compute spooff/plastic_strain_rate command requires atom_style with plastic_strain_rate (e.g. spooff)");
...
ComputeSPOOFFPlasticStrainRate::~ComputeSPOOFFPlasticStrainRate()
{
...
void ComputeSPOOFFPlasticStrainRate::init()
{

  int count = 0;
  for (int i = 0; i < modify->ncompute; i++)
    if (strcmp(modify->compute[i]->style,"spooff/plastic_strain_rate") == 0) count++;
  if (count > 1 && comm->me == 0)
    error->warning(FLERR,"More than one compute spooff/plastic_strain_rate");
}

/* ---------------------------------------------------------------------- */

void ComputeSPOOFFPlasticStrainRate::compute_peratom()
{
...
double ComputeSPOOFFPlasticStrainRate::memory_usage()
{
