...
/* ----------------------------------------------------------------------
 thermal EOS for use with linear elasticity and thermal expansion
 input: initial pressure pInitial, isotropic part of the strain rate d, time-step dt
 output: final pressure pFinal, pressure rate p_rate
 ------------------------------------------------------------------------- */
void ThermalEOS(double rho, double rho0, double e, double T, double T0, double bulk, double alpha, double cp, double pInitial, double dt,
                double &pFinal, double &p_rate) {

        double rho0T = rho0/(1+(3*alpha*(T-T0)));
        double mu = (rho / rho0T) - 1.0;

        pFinal = -bulk*mu;

        //printf("Thermal EOS: rho = %g, rho0 = %g, Alpha=%f, T0=%f, bulk=%f, e=%f, cp=%f\n", rho, rho0, alpha, T0, bulk, e, cp);
        //printf("pFinal = %f\n", pFinal);
...
bool IsotropicMaxStressDamage(const Matrix3d& S, const double maxStress) {
         ...
         */
        SelfAdjointEigenSolver < Matrix3d > es;
        es.compute(S); // compute eigenvalue and eigenvectors of stress
