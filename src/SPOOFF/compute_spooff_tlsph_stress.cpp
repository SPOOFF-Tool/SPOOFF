...
#include "compute_spooff_tlsph_stress.h"
#include <cmath>
#include <cstring>
#include <Eigen/Eigen>
#include "atom.h"
...
ComputeSPOOFFTLSPHStress::ComputeSPOOFFTLSPHStress(LAMMPS *lmp, int narg, char **arg) :
                Compute(lmp, narg, arg) {
        if (narg != 3)
                error->all(FLERR, "Illegal compute spooff/tlsph_stress command");
...
ComputeSPOOFFTLSPHStress::~ComputeSPOOFFTLSPHStress() {
        memory->sfree(stress_array);
}

/* ---------------------------------------------------------------------- */

void ComputeSPOOFFTLSPHStress::init() {

        int count = 0;
        for (int i = 0; i < modify->ncompute; i++)
                if (strcmp(modify->compute[i]->style, "spooff/tlsph_stress") == 0)
                        count++;
        if (count > 1 && comm->me == 0)
                error->warning(FLERR, "More than one compute spooff/tlsph_stress");
}

/* ---------------------------------------------------------------------- */

void ComputeSPOOFFTLSPHStress::compute_peratom() {
...
double ComputeSPOOFFTLSPHStress::memory_usage() {
