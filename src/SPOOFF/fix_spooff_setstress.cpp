...
#include "fix_spooff_setstress.h"
...
FixSPOOFFSetStress::FixSPOOFFSetStress(LAMMPS *lmp, int narg, char **arg) :
    Fix(lmp, narg, arg), xstr(nullptr), ystr(nullptr), zstr(nullptr), idregion(nullptr),
    region(nullptr), sforce(nullptr)
{
  if (narg < 6) error->all(FLERR, "Illegal fix setstress command");
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "region") == 0) {
      if (iarg + 2 > narg) error->all(FLERR, "Illegal fix setstress command");
      region = domain->get_region_by_id(arg[iarg + 1]);
      if (!region) error->all(FLERR, "Region {} for fix setstress does not exist", arg[iarg + 1]);
      idregion = utils::strdup(arg[iarg + 1]);
      iarg += 2;
    } else
      error->all(FLERR, "Illegal fix setstress command");
  }
  ...
  maxatom = atom->nmax;
  memory->create(sforce, maxatom, 3, "setstress:sforce");
}

/* ---------------------------------------------------------------------- */

FixSPOOFFSetStress::~FixSPOOFFSetStress()
{
  delete[] xstr;
  delete[] ystr;
  delete[] zstr;
...
int FixSPOOFFSetStress::setmask()
{
  int mask = 0;
  mask |= POST_FORCE;
  return mask;
...
void FixSPOOFFSetStress::init()
{
  // check variables

  if (xstr) {
    xvar = input->variable->find(xstr);
    if (xvar < 0) error->all(FLERR, "Variable name for fix setstress does not exist");
    if (input->variable->equalstyle(xvar))
      xstyle = EQUAL;
    else if (input->variable->atomstyle(xvar))
      xstyle = ATOM;
    else
      error->all(FLERR, "Variable for fix setstress is invalid style");
  }
  if (ystr) {
    yvar = input->variable->find(ystr);
    if (yvar < 0) error->all(FLERR, "Variable name for fix setstress does not exist");
  ...
  if (zstr) {
    ...
    if (zvar < 0) error->all(FLERR, "Variable name for fix setstress does not exist");
  ...
  if (idregion) {
    ...
    if (!region) error->all(FLERR, "Region {} for fix setstress does not exist", idregion);
  }
...
void FixSPOOFFSetStress::setup(int vflag)
{
  if (utils::strmatch(update->integrate_style, "^verlet"))
    post_force(vflag);
  else
    error->all(FLERR, "Fix spooff/setstress does not support RESPA");
}

/* ---------------------------------------------------------------------- */

void FixSPOOFFSetStress::min_setup(int vflag)
{
  post_force(vflag);
}
...
void FixSPOOFFSetStress::post_force(int /*vflag*/)
{
  double **x = atom->x;
  double **f = atom->f;
  double **v = atom->v;
  ...
  if (T == nullptr) {
                error->all(FLERR, "compute spooff/tlsph_stress could not access stress tensors. Are the matching pair styles present?");
        }
  ...
  if (varflag == CONSTANT) {
    ...
    for (int i = 0; i < nlocal; i++)
      if (mask[i] & groupbit) {
        ...
        T[i](2, 1) = 0.0; // zy
        if (xstyle == ATOM) {
          T[i](0, 0) = sforce[i][0]; // xx
        } else if (xstyle) {
          T[i](0, 0) = xvalue; // xx
        }

        if (ystyle == ATOM) {
          T[i](1, 1) = sforce[i][1]; // yy
        } else if (ystyle) {
          T[i](1, 1) = yvalue; // yy
        }

        if (zstyle == ATOM) {
          T[i](2, 2) = sforce[i][2]; // zz
        } else if (zstyle) {
          T[i](2, 2) = zvalue; // zz
        }
      }
...
double FixSPOOFFSetStress::compute_vector(int n)
{
...
double FixSPOOFFSetStress::memory_usage()
{
  double bytes = 0.0;
