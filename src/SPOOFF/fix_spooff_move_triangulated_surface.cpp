...
#include "fix_spooff_move_triangulated_surface.h"
...
FixSPOOFFMoveTriSurf::FixSPOOFFMoveTriSurf(LAMMPS *lmp, int narg, char **arg) :
                Fix(lmp, narg, arg) {

        if (atom->smd_flag != 1) {
                error->all(FLERR, "fix fix spooff/move_tri_surf command requires atom_style spooff");
        }

        if (narg < 3)
                error->all(FLERR, "Illegal number of arguments for fix fix spooff/move_tri_surf command");
        ...
        if (comm->me == 0) {
                printf("\n>>========>>========>>========>>========>>========>>========>>========>>========\n");
                printf("fix fix spooff/move_tri_surf is active for group: %s \n", arg[1]);
        }
        while (true) {
                ...
                if (strcmp(arg[iarg], "*LINEAR") == 0) {
                        ...
                        char msg[128];
                        snprintf(msg,128, "Illegal keyword for fix spooff/move_tri_surf: %s\n", arg[iarg]);
                        error->all(FLERR, msg);
                }
...
int FixSPOOFFMoveTriSurf::setmask() {
        int mask = 0;
...
void FixSPOOFFMoveTriSurf::init() {
        dtv = update->dt;
}
...
void FixSPOOFFMoveTriSurf::initial_integrate(int /*vflag*/) {
        double **x = atom->x;
        double **x0 = atom->x0;
        double **v = atom->v;
...
void FixSPOOFFMoveTriSurf::reset_dt() {
        dtv = update->dt;
}

/* ---------------------------------------------------------------------- */

int FixSPOOFFMoveTriSurf::pack_forward_comm(int n, int *list, double *buf, int /*pbc_flag*/, int * /*pbc*/) {
        int i, j, m;
        double **x0 = atom->x0;
        double **smd_data_9 = atom->smd_data_9;

        //printf("in FixSPOOFFIntegrateTlsph::pack_forward_comm\n");
        m = 0;
        for (i = 0; i < n; i++) {
                j = list[i];
                buf[m++] = x0[j][0];
...
void FixSPOOFFMoveTriSurf::unpack_forward_comm(int n, int first, double *buf) {
        int i, m, last;
        double **x0 = atom->x0;
        double **smd_data_9 = atom->smd_data_9;

        //printf("in FixSPOOFFMoveTriSurf::unpack_forward_comm\n");
        m = 0;
        last = first + n;
