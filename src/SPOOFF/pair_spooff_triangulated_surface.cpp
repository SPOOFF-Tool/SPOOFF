...
#include "pair_spooff_triangulated_surface.h"
...
void PairTriSurf::settings(int narg, char **arg) {
        if (narg != 1)
                error->all(FLERR, "Illegal number of args for pair_style spooff/tri_surface");

        scale = utils::numeric(FLERR, arg[0],false,lmp);
        if (comm->me == 0) {
                printf("\n>>========>>========>>========>>========>>========>>========>>========>>========\n");
                printf("SPOOFF/TRI_SURFACE CONTACT SETTINGS:\n");
...
double PairTriSurf::init_one(int i, int j) {
        ...
        if (comm->me == 0) {
                printf("cutoff for pair spooff/spooff/tri_surface = %f\n", cutoff);
        }
        return cutoff;
}
...
void PairTriSurf::init_style() {
        ...
        if (!atom->contact_radius_flag)
                error->all(FLERR, "Pair style spooff/spooff/tri_surface requires atom style with contact_radius");
...
void *PairTriSurf::extract(const char *str, int &/*i*/) {
        ...
        if (strcmp(str, "spooff/tri_surface/stable_time_increment_ptr") == 0) {
