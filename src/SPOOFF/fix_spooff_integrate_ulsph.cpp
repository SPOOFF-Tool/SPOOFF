...
#include "fix_spooff_integrate_ulsph.h"
#include <cmath>
#include <cstring>
#include <Eigen/Eigen>
#include "atom.h"
...
FixSPOOFFIntegrateUlsph::FixSPOOFFIntegrateUlsph(LAMMPS *lmp, int narg, char **arg) :
                Fix(lmp, narg, arg) {

        if ((atom->esph_flag != 1) || (atom->vfrac_flag != 1))
                error->all(FLERR, "fix spooff/integrate_ulsph command requires atom_style with both energy and volume");

        if (narg < 3)
                error->all(FLERR, "Illegal number of arguments for fix spooff/integrate_ulsph command");
        ...
        if (comm->me == 0) {
                printf("\n>>========>>========>>========>>========>>========>>========>>========>>========\n");
                printf("fix spooff/integrate_ulsph is active for group: %s \n", arg[1]);
        }
        while (true) {
                ...
                if (strcmp(arg[iarg], "xsph") == 0) {
                        ...
                        char msg[128];
                        snprintf(msg,128, "Illegal keyword for spooff/integrate_ulsph: %s\n", arg[iarg]);
                        error->all(FLERR, msg);
                }
...
int FixSPOOFFIntegrateUlsph::setmask() {
        int mask = 0;
...
void FixSPOOFFIntegrateUlsph::init() {
        dtv = update->dt;
...
void FixSPOOFFIntegrateUlsph::initial_integrate(int /*vflag*/) {
        double **x = atom->x;
        double **v = atom->v;
        double **f = atom->f;
        ...
        auto smoothVel = (Vector3d *) force->pair->extract("spooff/ulsph/smoothVel_ptr", itmp);

        if (xsphFlag) {
                if (smoothVel == nullptr) {
                        error->one(FLERR, "fix spooff/integrate_ulsph failed to access smoothVel array");
                }
        }
...
void FixSPOOFFIntegrateUlsph::final_integrate() {
        double **v = atom->v;
        double **f = atom->f;
        ...
        int itmp;
        int *nn = (int *) force->pair->extract("spooff/ulsph/numNeighs_ptr", itmp);
        if (nn == nullptr) {
                error->one(FLERR, "fix spooff/integrate_ulsph failed to accesss num_neighs array");
        }

        auto L = (Matrix3d *) force->pair->extract("spooff/ulsph/velocityGradient_ptr", itmp);
        if (L == nullptr) {
                error->one(FLERR, "fix spooff/integrate_ulsph failed to accesss velocityGradient array");
        }
...
void FixSPOOFFIntegrateUlsph::reset_dt() {
        dtv = update->dt;
        dtf = 0.5 * update->dt * force->ftm2v;
}
