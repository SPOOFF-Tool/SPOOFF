...
#include <cstring>
#include <Eigen/Eigen>
#include "compute_spooff_ulsph_strain_rate.h"
#include "atom.h"
#include "update.h"
#include "modify.h"
#include "comm.h"
...
ComputeSPOOFFULSPHStrainRate::ComputeSPOOFFULSPHStrainRate(LAMMPS *lmp, int narg, char **arg) :
...
ComputeSPOOFFULSPHStrainRate::~ComputeSPOOFFULSPHStrainRate() {
        memory->sfree(strain_rate_array);
}

/* ---------------------------------------------------------------------- */

void ComputeSPOOFFULSPHStrainRate::init() {
...
void ComputeSPOOFFULSPHStrainRate::compute_peratom() {
        ...
        if (L == nullptr) {
                error->all(FLERR,
                                "compute spooff/ulsph_strain_rate could not access any velocity gradients. Are the matching pair styles present?");
        }
...
double ComputeSPOOFFULSPHStrainRate::memory_usage() {
