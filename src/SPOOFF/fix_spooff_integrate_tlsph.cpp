...
#include "fix_spooff_integrate_tlsph.h"
#include <cmath>
#include <cstring>
#include <Eigen/Eigen>
#include "atom.h"
...
FixSPOOFFIntegrateTlsph::FixSPOOFFIntegrateTlsph(LAMMPS *lmp, int narg, char **arg) :
                Fix(lmp, narg, arg) {
        if (narg < 3) {
                printf("narg=%d\n", narg);
                error->all(FLERR, "Illegal fix spooff/integrate_tlsph command");
        }
        ...
        if (comm->me == 0) {
                printf("\n>>========>>========>>========>>========>>========>>========>>========>>========\n");
                printf("fix spooff/integrate_tlsph is active for group: %s \n", arg[1]);
        }
        ...
        while (true) {
                ...
                if (strcmp(arg[iarg], "xsph") == 0) {
                        ...
                        char msg[128];
                        snprintf(msg,128, "Illegal keyword for spooff/integrate_tlsph: %s\n", arg[iarg]);
                        error->all(FLERR, msg);
                }
...
int FixSPOOFFIntegrateTlsph::setmask() {
        int mask = 0;
...
void FixSPOOFFIntegrateTlsph::init() {
        dtv = update->dt;
...
void FixSPOOFFIntegrateTlsph::initial_integrate(int /*vflag*/) {
        ...
        auto smoothVelDifference = (Vector3d *) force->pair->extract("spooff/tlsph/smoothVel_ptr", itmp);

        if (xsphFlag) {
                if (smoothVelDifference == nullptr) {
                        error->one(FLERR,
                                        "fix spooff/integrate_tlsph failed to access smoothVel array. Check if a pair style exist which calculates this quantity.");
                }
        }
...
void FixSPOOFFIntegrateTlsph::final_integrate() {
        double dtfm, vsq, scale;
...
// update v of atoms in group
        ...
        for (i = 0; i < nlocal; i++) {
                if (mask[i] & groupbit) {
                        ...
                        esph[i] += dtv * desph[i];
                        pesph[i] += dtv * dpesph[i];
                        tsph[i] += dtv * desph[i]/(cpsph[i]*rmass[i]);
                }
        }
}
...
void FixSPOOFFIntegrateTlsph::reset_dt() {
        dtv = update->dt;
        dtf = 0.5 * update->dt * force->ftm2v;
        vlimitsq = vlimit * vlimit;
}
