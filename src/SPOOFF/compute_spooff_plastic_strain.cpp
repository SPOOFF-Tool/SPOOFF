...
#include "compute_spooff_plastic_strain.h"
#include <cstring>
#include "atom.h"
#include "update.h"
#include "modify.h"
...
ComputeSPOOFFPlasticStrain::ComputeSPOOFFPlasticStrain(LAMMPS *lmp, int narg, char **arg) :
  Compute(lmp, narg, arg)
{
  if (narg != 3) error->all(FLERR,"Illegal compute spooff/plastic_strain command");
  if (atom->eff_plastic_strain_flag != 1) error->all(FLERR,"compute spooff/plastic_strain command requires atom_style with plastic_strain (e.g. spooff)");
...
ComputeSPOOFFPlasticStrain::~ComputeSPOOFFPlasticStrain()
{
...
void ComputeSPOOFFPlasticStrain::init()
{

  int count = 0;
  for (int i = 0; i < modify->ncompute; i++)
    if (strcmp(modify->compute[i]->style,"spooff/plastic_strain") == 0) count++;
  if (count > 1 && comm->me == 0)
    error->warning(FLERR,"More than one compute spooff/plastic_strain");
}

/* ---------------------------------------------------------------------- */

void ComputeSPOOFFPlasticStrain::compute_peratom()
{
...
double ComputeSPOOFFPlasticStrain::memory_usage()
{
